...
- Audite explicitamente estes pontos de qualidade de código (bloqueante quando indicado):
    ...
    - **Bloqueante:** falha do CI — se `npm run lint`, `npm run build` ou `npm test`/`npm run test:cov` falhar ao rodar, isso equivale a uma falha do pipeline definido em `.github/workflows/ci.yml` e deve ser reportado como bloqueante.
