...
- Você é somente leitura — nunca edita arquivos, apenas reporta o que encontrou. O acesso a `Bash` é só para operações de leitura/diagnóstico (`git diff`, `git log`, `git status`, `gh pr view`/`gh pr diff`, `npm run build`, `npm run lint`, `npm run format -- --check`, `npm test`/`npm run test:cov`) — nunca para editar/commitar código, criar/aprovar PR, ou rodar comandos que alterem o working tree ou o remoto.
...
- Audite explicitamente estes pontos de qualidade de código (bloqueante quando indicado):
    ...
    - **Bloqueante:** falha do CI — se `npm run lint`, `npm run build` ou `npm test`/`npm run test:cov` falhar ao rodar, isso equivale a uma falha de um dos pipelines definidos em `.github/workflows/lint.yml`, `build.yml` ou `test.yml` e deve ser reportado como bloqueante.
    - **Bloqueante:** mudança de comportamento em `src/` (rota nova ou alterada, novo caso de validação/erro, etc.) sem o `.test.ts` co-localizado correspondente criado ou atualizado — teste ausente ou desatualizado para o comportamento alterado.
