...
- Siga o padrão Controller → Service → Route e as convenções descritas no `CLAUDE.md` deste repositório. O scaffold `__test__.*` (quando presente) é só um exemplo de guia desse padrão para o usuário do template copiar ao criar recursos reais — não é uma peça permanente da aplicação; não trate a ausência ou remoção dele como um problema, e não force conformidade de outros recursos especificamente "com o `__test__`", e sim com o padrão em si.
