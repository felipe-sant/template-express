...
- `npm run test:cov` — roda a suíte com cobertura (`vitest run --coverage`); falha se qualquer métrica (`lines`/`branches`/`functions`/`statements`) ficar abaixo de 80%, conforme `test.coverage.thresholds` em `vitest.config.mts`.

O test runner é `vitest`, com `supertest` para integração HTTP (requisições reais contra o app Express nos testes de rota). Convenção: teste co-localizado com sufixo `.test.ts` ao lado do arquivo testado (ex.: `src/routes/preview.routes.test.ts`) — nunca `__tests__/` nem `.spec.ts`. Escopo de teste é só `src/` (descoberta via `include: ["src/**/*.test.ts"]` em `vitest.config.mts`; cobertura via `coverage.include: ["src/**/*.ts"]`), não `out/` nem arquivos de config. Cobertura mínima de 80% (lines/branches/functions/statements) é obrigatória via `npm run test:cov`. Testar uma rota segue a convenção de **cobertura exaustiva de cenários**: um caso de teste separado para cada cenário de validação/erro que o `Controller` daquela rota realmente trata (sem inventar cenário que o controller não trata, nem deixar de cobrir um que trata), mais o caminho completo/happy path — "sucesso + um erro genérico" não é suficiente. Ver skill `vitest-specialist` para o passo a passo detalhado (TDD Red→Green→Refactor, padrões por camada, mocking).
